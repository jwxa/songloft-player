...
void main() {
  ...
  String wrap(Uri abs) =>
      'http://127.0.0.1:9/w?u=${Uri.encodeComponent(abs.toString())}';

  final base = Uri.parse(
    'https://self-signed.example/api/v1/songs/5/play.m3u8',
  );
  ...
  group('rewriteHlsPlaylist', () {
    test('相对切片行按 base 解析并改写', () {
      ...
      expect(
        out,
        contains(
          'u=${Uri.encodeComponent('https://self-signed.example/api/v1/songs/5/seg0.ts')}',
        ),
      );
      expect(
        out,
        contains(
          'u=${Uri.encodeComponent('https://self-signed.example/api/v1/songs/5/sub/seg1.ts')}',
        ),
      );
      // 标签行保持不变
    ...
    test('后端 hls-proxy 风格的相对 segment?u= 保留查询串', () {
      ...
      expect(
        out,
        contains(
          'u=${Uri.encodeComponent('https://self-signed.example/api/v1/songs/5/hls/segment?u=aHR0cHM6Ly9vcmln')}',
        ),
      );
    });
    ...
    test('EXT-X-KEY 的 URI 属性被改写', () {
      ...
      expect(
        out,
        contains(
          'URI="http://127.0.0.1:9/w?u=${Uri.encodeComponent('https://self-signed.example/api/v1/songs/5/keys/k1.key')}"',
        ),
      );
      // METHOD / IV 等其它属性保持不变
    ...
    test('EXT-X-MAP 与变体播放列表 URI 被改写', () {
      ...
      expect(
        out,
        contains(
          'URI="http://127.0.0.1:9/w?u=${Uri.encodeComponent('https://self-signed.example/api/v1/songs/5/init.mp4')}"',
        ),
      );
      expect(
        out,
        contains(
          'u=${Uri.encodeComponent('https://self-signed.example/api/v1/songs/5/720p.m3u8')}',
        ),
      );
    });
